...
Event discovery is a read-only query module, separate from the Event command interface. It lists Public Events through required-city, optional district/Category/date queries and a stable `starts_at, id` cursor (maximum fifty results). Upcoming contains future Published Events; History preserves completed and cancelled plans. It projects the caller's own Attendance status, capacity, available seats, route summary, and authorized map marker; attendee identities are only projected in detail under the roster/profile rules. Inactive Categories remain on existing Event cards but do not appear as filter choices. Unlisted and Private detail authorization is evaluated before projection; general discovery never returns either. A Public Event remains directly viewable after cancellation or completion but exposes no join action. A Personal Calendar has the equivalent Upcoming and History views. Free-text search, rankings, radius search, recommendations, cache, and an external search adapter are intentionally absent.
