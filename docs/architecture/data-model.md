...
General Event Discovery reads Public Events by city, with optional district, Category, and date-range filters. Its Upcoming view contains future Published Events; its History view preserves completed and cancelled Public plans. Results sort by `starts_at, id` and use that tuple as a cursor, with at most fifty results per page. Unlisted and Private Events never enter general discovery. A signed-in User may receive only their own Attendance status beside an otherwise discoverable Event. A detail can project the allowed attendee roster: the Organizer can see confirmed participants, while confirmed participants can see profiles according to each Profile's visibility rule. An inactive Category remains discoverable through its existing Event but is not offered as a Category filter. A Public Event remains directly viewable after completion or cancellation, with its status and no join action. Discovery may also present map markers for events whose location visibility permits it; it is not a radius/recommendation search engine.

A Personal Calendar returns Organizer Events and active Attendance (`CONFIRMED`, `PENDING`, or `WAITLISTED`) in Upcoming and History views. Cancelled and completed events remain readable with their status.

Organizer transfer is explicit: the current Organizer may request a transfer to another Confirmed participant of a scheduled Published Event. A single pending transfer exists per Event. Acceptance atomically replaces `organizer_id` without changing attendance or capacity; decline leaves ownership unchanged. A finite capacity increase promotes the oldest eligible Waitlisted Attendances up to the number of newly available seats. Changing capacity to unlimited confirms every eligible Waitlisted Attendance for an `OPEN` Event in the same transaction; an `APPROVAL_REQUIRED` Event preserves its Organizer decision rule.
...
The Organizer's initial Confirmed Attendance cannot be cancelled independently. An Organizer who wants to leave a multi-participant Event can first transfer organization to an eligible participant, then cancel their own Attendance; otherwise they cancel the Event.
