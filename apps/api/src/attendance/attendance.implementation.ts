...
export class AttendanceImplementation implements AttendanceModule {
  ...
  async decide(command: RequestAttendance | EnrollWaitlist | CancelAttendance | DecideAttendance | AcceptInvitation): Promise<AttendanceOutcome> {
    if (command.kind === 'ENROLL_WAITLIST') return this.enrollWaitlist(command);
    if (command.kind === 'CANCEL_ATTENDANCE') return this.cancelAttendance(command);
    if (command.kind === 'DECIDE_ATTENDANCE') { const outcome = await this.decideAttendance(command); await this.publishDecision(command, outcome); return outcome; }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const status = isApprovalRequired ? 'PENDING' : 'CONFIRMED';
      const now = this.now();
  ...
  private async cancelAttendance(command: CancelAttendance): Promise<AttendanceOutcome> { return this.dataSource.transaction(async (manager) => { const event = await manager.findOne(EventRecord, { where: { id: command.eventId }, lock: { mode: 'pessimistic_write' } }); if (!event) throw new AttendanceBusinessError('EVENT_NOT_JOINABLE'); if (event.organizerId === command.actorUserId) throw new AttendanceBusinessError('FORBIDDEN'); const row = await manager.findOneBy(AttendanceRecord, { eventId: event.id, userId: command.actorUserId }); if (!row) throw new AttendanceBusinessError('INVALID_ATTENDANCE_TRANSITION'); if (row.status === 'CANCELLED') return this.outcome(row, event, 'CANCELLED'); if (row.status !== 'CONFIRMED' && (event.startsAt <= this.now() || (row.status !== 'PENDING' && row.status !== 'WAITLISTED'))) throw new AttendanceBusinessError('INVALID_ATTENDANCE_TRANSITION'); row.status = 'CANCELLED'; row.cancelledAt = this.now(); row.version += 1; row.updatedByUserId = command.actorUserId; await manager.save(row); if (row.confirmedAt) event.confirmedCount -= 1; if (event.status === 'PUBLISHED' && event.joinPolicy === 'OPEN' && event.startsAt > this.now() && row.confirmedAt) { const next = await manager.findOne(AttendanceRecord, { where: { eventId: event.id, status: 'WAITLISTED' }, order: { waitlistedAt: 'ASC', id: 'ASC' }, lock: { mode: 'pessimistic_write' } }); if (next && (event.capacity === null || event.confirmedCount < event.capacity)) { next.status = 'CONFIRMED'; next.confirmedAt = this.now(); next.version += 1; await manager.save(next); event.confirmedCount += 1; } } await manager.save(event); return this.outcome(row, event, 'CANCELLED'); }); }
