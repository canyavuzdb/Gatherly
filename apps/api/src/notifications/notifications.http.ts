...
export class NotificationsHttpController {
  ...
  @Get() async list(@Headers('authorization') authorization: string | undefined, @Query('before') before?: string, @Query('limit') limit?: string) { return this.withActor(authorization, (actor) => this.notifications.list({ actor, before, limit: limit === undefined ? undefined : Number(limit) }), false); }
  ...
  private async withActor<T>(authorization: string | undefined, operation: (actor: { userId: string; verification: 'UNVERIFIED' | 'VERIFIED' }) => Promise<T>, recordActivity = true) { const token = /^Bearer (.+)$/.exec(authorization ?? '')?.[1]; if (!token) throw new UnauthorizedException('ACCESS_TOKEN_INVALID'); try { return await operation(await this.auth.authenticate(token, { recordActivity })); } catch (error) { if (error instanceof AuthBusinessError) throw new UnauthorizedException(error.code); if (error instanceof NotificationsBusinessError) { if (error.code === 'NOTIFICATION_NOT_FOUND_OR_NOT_OWNED') throw new NotFoundException(error.code); throw new BadRequestException(error.code); } throw error; } }
}
