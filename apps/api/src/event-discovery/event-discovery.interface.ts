...
export type EventCard = { id: string; title: string; startsAt: Date; endsAt: Date; timezone: string; status: 'PUBLISHED' | 'CANCELLED' | 'COMPLETED'; category: { id: string; name: string; isActive: boolean }; location: { city: string; district: string; venueName: string | null }; capacity: CapacityView; ownAttendanceStatus?: 'PENDING' | 'CONFIRMED' | 'WAITLISTED' | 'REJECTED' | 'CANCELLED' };
...
export type EventDetail = Omit<EventCard, 'status'> & { status: 'DRAFT' | 'PUBLISHED' | 'CANCELLED' | 'COMPLETED'; description: string; visibility: 'PUBLIC' | 'UNLISTED' | 'PRIVATE'; joinPolicy: 'OPEN' | 'APPROVAL_REQUIRED' | 'INVITE_ONLY'; location: EventCard['location'] & { address: string | null }; joinAvailable: boolean };
export type PersonalCalendar = { actor: UserIdentity; after?: string; limit?: number };
export type CalendarEventCard = Omit<EventCard, 'status'> & { status: 'DRAFT' | 'PUBLISHED' | 'CANCELLED'; relationship: 'ORGANIZER' | 'ATTENDEE' };
