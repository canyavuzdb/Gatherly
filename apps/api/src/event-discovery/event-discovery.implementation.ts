...
export class EventDiscoveryImplementation implements EventDiscoveryModule {
  ...
  async personalCalendar(request: PersonalCalendar): Promise<CalendarPage> {
    ...
    const rows = await this.dataSource.getRepository(EventRecord).createQueryBuilder('event')
      ...
      .where('event.starts_at > :now', { now: this.now() }).andWhere("event.status IN ('DRAFT','PUBLISHED','CANCELLED')").andWhere('(event.organizer_id = :userId OR attendance.id IS NOT NULL)', { userId: request.actor.userId })
      .andWhere(cursor ? '(event.starts_at, event.id) > (:cursorStartsAt, :cursorId)' : 'true', cursor ? { cursorStartsAt: cursor.startsAt, cursorId: cursor.id } : {})
      .orderBy('event.starts_at', 'ASC').addOrderBy('event.id', 'ASC').limit(limit + 1).getRawMany();
