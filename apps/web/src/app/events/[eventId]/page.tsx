...
type EventDetail = {
  ...
  ownParticipationOutcome?: 'ATTENDED' | 'NO_SHOW';
...
export default function EventDetailPage() {
  ...
  const [recentReviews, setRecentReviews] = useState<Array<{ rating: number; comment: string; createdAt: string }>>([]);
  ...
  async function loadReviews() {
    ...
    const reviews = await response.json() as { summary: Array<{ subject: 'EVENT' | 'ORGANIZER'; count: number; average: number | null }>; own: Array<{ subject: 'EVENT' | 'ORGANIZER'; rating: number; comment: string | null }>; recent: Array<{ rating: number; comment: string; createdAt: string }> };
    setReviewSummary(reviews.summary); setOwnReviews(reviews.own); setRecentReviews(reviews.recent);
  ...
  return <AppSidebar>
    <article className="event-detail">
      ...
      {event.organizerPreview.kind === 'VISIBLE' ? <Link className="event-organizer" href={`/people/${event.organizerPreview.userId}`} aria-label={`${event.organizerPreview.name} profili`}><div className="participant-avatar"><span>{event.organizerPreview.initials}</span>{event.organizerPreview.avatarMediaAssetId && <img src={mediaUrl(event.organizerPreview.avatarMediaAssetId)} alt={event.organizerPreview.name} onError={(image) => { image.currentTarget.style.display = 'none'; }} />}</div><div><p className="auth-eyebrow">Organizatör</p><strong>{event.organizerPreview.name}</strong><small>Profil ve değerlendirmeleri gör →</small></div></Link> : <section className="event-organizer" aria-label="Etkinlik organizatörü"><div className="participant-avatar is-anonymous"><span>{event.organizerPreview.initials}</span></div><div><p className="auth-eyebrow">Organizatör</p><strong>Gizli profil</strong></div></section>}
...
function formatReviewDate(createdAt: string) { return new Intl.DateTimeFormat('tr-TR', { dateStyle: 'medium' }).format(new Date(createdAt)); }
