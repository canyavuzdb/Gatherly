...
export default function NewEventPage() {
  ...
  useEffect(() => {
    ...
    async function loadCategories() {
      try {
        const response = await authenticatedFetch('/api/v1/events?city=Istanbul&limit=1');
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const definition = { categoryId: form.categoryId, title: form.title, description: form.description, startsAt: new Date(form.startsAt).toISOString(), endsAt: new Date(form.endsAt).toISOString(), timezone: 'Europe/Istanbul', capacity: form.capacity ? Number(form.capacity) : null, visibility: form.visibility, joinPolicy: form.joinPolicy, location: { city: form.city, district: form.district, venueName: form.venueName.trim() || null, address: form.address.trim() || null, latitude: form.latitude, longitude: form.longitude, routeMode: form.routeMode, routeEndLatitude: form.routeEndLatitude, routeEndLongitude: form.routeEndLongitude, addressVisibility: form.addressVisibility } };
      const draftResponse = await authenticatedFetch('/api/v1/events', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(definition) });
      ...
      const publishResponse = await authenticatedFetch(`/api/v1/events/${draft.event.id}/publish`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ expectedVersion: draft.event.version }) });
  ...
  return <AppSidebar><section className="event-create-stage"><div><p className="auth-eyebrow">YENİ ETKİNLİK</p><h1 className="dashboard-title">Bir araya gelin.</h1><p>Etkinliğin yayınlandığında seçtiğin şehir takviminde saat sırasıyla görünür.</p></div><form className="event-create-form" onSubmit={submit}>
    <label className="field event-create-wide"><span className="field-label is-required">Etkinlik adı</span><input className="field-input" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} maxLength={160} required /></label>
    <label className="field event-create-wide"><span className="field-label is-required">Açıklama</span><textarea className="field-input event-create-description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} required /></label>
    <label className="field"><span className="field-label is-required">Kategori</span><select className="field-input" value={form.categoryId} onChange={(event) => setForm({ ...form, categoryId: event.target.value })} disabled={!categories.length} required>{categories.length ? categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>) : <option>Kategori yükleniyor…</option>}</select></label>
    ...
    <label className="field"><span className="field-label is-required">Başlangıç</span><input className="field-input" type="datetime-local" value={form.startsAt} onChange={(event) => setForm({ ...form, startsAt: event.target.value })} required /></label>
    <label className="field"><span className="field-label is-required">Bitiş</span><input className="field-input" type="datetime-local" value={form.endsAt} onChange={(event) => setForm({ ...form, endsAt: event.target.value })} required /></label>
    <label className="field"><span className="field-label is-required">Şehir</span><select className="field-input" value={form.city} onChange={(event) => setForm({ ...form, city: event.target.value })}>{CITY_OPTIONS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
    <label className="field"><span className="field-label is-required">İlçe</span><input className="field-input" value={form.district} onChange={(event) => setForm({ ...form, district: event.target.value })} required /></label>
...
async function uploadAndAttach(token: string, eventId: string, file: File, role: 'COVER' | 'GALLERY', altText: string) {
  ...
  const uploadResponse = await authenticatedFetch('/api/v1/media/images', { method: 'POST', body: formData });
  ...
  const attachResponse = await authenticatedFetch(`/api/v1/events/${eventId}/media`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ mediaAssetId: uploaded.mediaAsset.id, role, altText }) });
